...
export function CommandCenter() {
  ...
  const contadoresMap = useMemo(() => ({
    faturamento: (
      <WidgetContador
        label="Faturamento do mês" icon={DollarSign} destaque
        valor={formatarMoeda(kpis.faturamento)}
        variacao={variacaoFaturamento}
        variacaoLabel="vs mês anterior"
        subvalor={`${resumoFrascosMes.pedidos} pedidos pagos`}
      />
    ),
    frascos: (
      <WidgetContador
        label="Frascos vendidos" icon={Package}
        valor={String(resumoFrascosMes.frascos)}
        subvalor={`${resumoFrascosMes.pedidos} pedido${resumoFrascosMes.pedidos !== 1 ? 's' : ''} no mês`}
      />
    ),
    estoque: (
      <WidgetContador
  ...
  }), [kpis.faturamento, variacaoFaturamento, resumoFrascosMes, alertaEstoque, estoqueProduto, leadsB2BAtivos])
        ...
        <section className="flex flex-col gap-4">
          ...
          <div className="cards-gap grid-mobile-1" style={{ gridTemplateColumns: 'repeat(4, 1fr)' }}>
            <div style={{ gridColumn: isMobile ? undefined : 'span 2' }}>
              <WidgetLinha title="Vendas por dia — últimos 30 dias" icon={Activity} data={vendas30dias} formatarValor={(v) => formatarMoeda(v)} />
            </div>
